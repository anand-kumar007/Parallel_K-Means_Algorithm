...
void Node::scatterDataset()
{
    ...
    int pointsPerNode[numNodes], datasetDisp[numNodes];
...
int Node::getIdNearestCluster(Point p)
{
    ...
    if (distance == 1)
    {
        ...
        double sum = squared_norm(clusters[0], p);
    ...
    else if (distance == 2)
    {
        ...
        for (int k = 0; k < K; k++)
        {
            double sim = cosine_similarity(clusters[k], p);
            if (sim > max_sim)
            {
                max_sim = sim;
                idCluster = k;
...
void Node::computeGlobalMembership()
{
    ...
    int localMem[numPoints], globalMember[numPoints];
...
{
    double *executionTimes, *ompExecTimes;
    int numNodes;
    MPI_Comm_size(comm, &numNodes);
